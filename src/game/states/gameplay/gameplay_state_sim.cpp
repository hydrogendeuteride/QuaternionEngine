...
namespace Game
{
                                                      ...
                                                      const bool thrusting)
    {
        ...
        std::vector<orbitsim::TrajectorySample> fallback_trajectory;
        fallback_trajectory.reserve(static_cast<size_t>(steps) + 1);

        glm::dvec3 p_rel_m = ship_rel_pos_m;
        glm::dvec3 v_rel_mps = ship_rel_vel_mps;
        const bool can_use_nbody_fallback = ref_sim != nullptr;

        for (int i = 0; i <= steps; ++i)
        {
            orbitsim::TrajectorySample sample{};
            sample.t_s = static_cast<double>(i) * dt_s;
            sample.position_m = p_rel_m;
            sample.velocity_mps = v_rel_mps;
            fallback_trajectory.push_back(sample);
            ...
            {
                a_rel = point_mass_accel(_orbitsim->sim.config().gravitational_constant, ref_info->mass_kg, p_rel_m, 0.0);
            }
        ...
        (void) fill_cache_from_trajectory(std::move(fallback_trajectory));
    }
    ...
    void GameplayState::emit_orbit_prediction_debug(GameStateContext &ctx)
    {
        ...
        const double fixed_dt_s = static_cast<double>(ctx.fixed_delta_time());
        ...
        {
            now_s -= (1.0 - static_cast<double>(alpha_f)) * fixed_dt_s;
        }
