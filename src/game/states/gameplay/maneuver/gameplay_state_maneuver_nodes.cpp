...
namespace Game
{
    ...
    void GameplayState::refresh_maneuver_node_runtime_cache(GameStateContext &ctx)
    {
        ...
        const bool chunk_only_preview_overlay =
                preview_chunk_assembly &&
                preview_prediction_cache &&
                !player_track->preview_overlay.has_flat_planned_cache();
        const OrbitPredictionCache *prediction_cache =
                chunk_only_preview_overlay ? preview_prediction_cache : display_prediction_cache;
        if (!display_prediction_cache || !display_prediction_cache->valid || display_prediction_cache->trajectory_frame.size() < 2)
        {
            return;
        }
        if (!prediction_cache || !prediction_cache->valid)
        {
            return;
        }

        const auto &traj_base = display_prediction_cache->trajectory_frame;
        const auto &traj_planned = display_prediction_cache->trajectory_frame_planned;
        ...
        const WorldVec3 align_delta = compute_maneuver_align_delta(ctx, *display_prediction_cache, traj_base);
        ...
        const auto *stable_traj_node_inertial =
                (stable_prediction_cache && stable_prediction_cache->trajectory_inertial_planned.size() >= 2)
                        ...
                        : nullptr;
        const bool allow_base_trajectory_node_fallback = _maneuver_state.nodes.size() <= 1;
        ...
        const orbitsim::TrajectoryFrameSpec display_frame_spec =
                display_prediction_cache->resolved_frame_spec_valid
                    ? display_prediction_cache->resolved_frame_spec
                    : resolve_prediction_display_frame_spec(*display_prediction_cache, now_s);
        ...
        for (ManeuverNode &node : _maneuver_state.nodes)
        {
            ...
            const OrbitPredictionCache *node_display_cache =
                    use_stable_planned_prefix_for_node ? stable_prediction_cache : display_prediction_cache;
            ...
            const std::vector<orbitsim::TrajectorySample> *node_traj_world =
                    use_stable_planned_prefix_for_node
                            ? stable_traj_node_world
                            : (traj_planned.size() >= 2
                                       ? &traj_planned
                                       : (stable_traj_node_world
                                                  ? stable_traj_node_world
                                                  : (allow_base_trajectory_node_fallback ? &traj_base : nullptr)));
            const std::vector<orbitsim::TrajectorySample> *node_traj_inertial =
                    use_stable_planned_prefix_for_node
                            ...
                            : (prediction_cache->trajectory_inertial_planned.size() >= 2
                                       ...
                                       : (stable_traj_node_inertial
                                                  ? stable_traj_node_inertial
                                                  : (allow_base_trajectory_node_fallback
                                                             ? &prediction_cache->trajectory_inertial
                                                             : nullptr)));
            ...
            if (!use_stable_planned_prefix_for_node && stable_planned_prefix_available)
            {
                const auto stable_preview_it = stable_preview_by_node_id.find(node.id);
                const bool active_preview_available = preview && preview->valid;
                const bool stable_preview_available =
                        stable_preview_it != stable_preview_by_node_id.end() &&
                        stable_preview_it->second &&
                        stable_preview_it->second->valid;
                const bool active_traj_covers_node_time =
                        node_traj_world &&
                        !node_traj_world->empty() &&
                        node.time_s >= node_traj_world->front().t_s &&
                        node.time_s <= node_traj_world->back().t_s;
                const bool stable_traj_covers_node_time =
                        stable_traj_node_world &&
                        !stable_traj_node_world->empty() &&
                        node.time_s >= stable_traj_node_world->front().t_s &&
                        node.time_s <= stable_traj_node_world->back().t_s;
                const bool use_stable_planned_fallback_for_node =
                        (!active_preview_available && stable_preview_available) ||
                        (!active_preview_available && !active_traj_covers_node_time && stable_traj_covers_node_time);
                if (use_stable_planned_fallback_for_node)
                {
                    node_prediction_cache = stable_prediction_cache;
                    node_display_cache = stable_prediction_cache;
                    ...
                    node_preview_chunk_assembly = nullptr;
            ...
            if (preview && preview->valid)
            {
                ...
                const orbitsim::MassiveBody *primary_body =
                        find_massive_body(node_prediction_cache->massive_bodies, primary_body_id);
                if (primary_body)
                {
