...
- The panel and gizmo both edit the same underlying node data: `ManeuverNode::dv_rpn_mps` and `ManeuverNode::time_s`.
- `dv_rpn_mps` stores maneuver-frame components in `Radial / Prograde / Normal` order.
- The gizmo and execution path use the same prograde-aligned maneuver frame, and the prediction worker converts that frame into solver RTN before building planned trajectories.
